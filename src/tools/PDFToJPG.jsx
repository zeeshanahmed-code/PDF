  ...
  const convert = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        images.push({ name: `page_${i}.jpg`, url: dataUrl, page: i })
        setProgress(Math.round((i / pdf.numPages) * 100))
      }

      setResults(images)
    } catch (e) {
      setError('Failed to convert PDF to JPG. Make sure the file is a valid PDF.')
    }
    setProcessing(false)
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-500 text-gray-700 mb-1">Quality: {Math.round(quality * 100)}%</label>
          <input type="range" min="0.5" max="1" step="0.05" value={quality} onChange={e => setQuality(Number(e.target.value))} className="w-full" />
        </div>
        <div>
          <label className="block text-sm font-500 text-gray-700 mb-1">Resolution: {scale}x</label>
      ...
      >
        {processing ? (
          <><span className="spinner w-5 h-5 border-2 border-white border-t-transparent rounded-full" />Converting... {progress}%</>
        ) : 'Convert to JPG'}
      </button>
      ...
      {results.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-500 text-gray-700">{results.length} image{results.length > 1 ? 's' : ''} ready</p>
            {results.length > 1 && (
              <button onClick={downloadAll} className="text-sm text-blue-600 hover:text-blue-700 font-500">Download All</button>
            )}
          </div>
            ...
            {results.map((r, i) => (
              <div key={i} className="relative group rounded-xl overflow-hidden border border-gray-200 aspect-[3/4]">
                ...
                <a
                  href={r.url}
                  download={r.name}
                  className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 text-white text-sm font-500 transition-opacity"
                >
